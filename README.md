...
In active development. Milestones 1–8 are complete: scaffolding, the app shell (sidebar/mobile-drawer navigation across all 7 screens) backed by a seeded, relationally-consistent mock API (MSW) with pagination/filtering/search/sort, the shared component library (EmptyState, ErrorState, Skeletons, StatusBadge, KPICard, ConfirmDialog, ActivityTimeline, useUrlFilters, and DataTable with sorting/pagination/column visibility/row selection/bulk actions/loading-error-empty states), full vertical slices for Vehicles, Drivers, Deliveries, and Maintenance (tables with URL-synced filters, detail pages, state-dependent actions with optimistic mutations and toast feedback), and the Dashboard (KPI cards, delivery-status chart with a Today/7d/30d toggle, fleet-status breakdown, top alerts, recent activity). Alerts and Dispatch remain, per Milestones 9–10.
