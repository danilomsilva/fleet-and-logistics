...
| **Vehicles / Drivers** | Sortable, filterable, searchable tables with column visibility and row selection + bulk status actions; detail pages with tabbed history (maintenance, deliveries, activity). |
...
Two real, non-obvious bugs surfaced during development and are worth calling out because they were genuinely tricky to diagnose:
