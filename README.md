...
| **Dispatch** | A MapLibre GL map of pickup locations for new (not yet picked up) deliveries, a "New deliveries available" panel, and a Zustand-backed assignment wizard — the dispatcher only picks a driver, since every driver already has their own vehicle assigned; conflict prevention happens at the data layer, offering only drivers the API reports as actually available. |
